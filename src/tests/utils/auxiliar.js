...
const validateWorkflow = (base, target) => {
  ...
  expect(base.blueprint_spec).toMatchObject(target.blueprint_spec);
  expect(target.created_at).toBeDefined();
};

const validateProcess = (
  process, workflow_id, blueprint_spec ) => {
...
const validateProcessState = (
  state, process_id, step_number, node_id, next_node_id, bag,
...
const validateTask = (
  task, workflow_name, process_status, node_name,
  lane_name, process_step_number) => {
...
const validateTaskWithWorkflow = (task, workflow, process_status) => {
  ...
  validateTask(task, workflow.name, process_status, node.id,
                lane.name, step_number);
};
