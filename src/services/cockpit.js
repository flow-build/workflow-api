...
async function fetchWorkflowProcessesByWorkflowName(workflow_name, status = null , limit = 20 , offset = 0 ) {
    ...
    .select('process.id','process.created_at','process.current_state_id','process.current_status','workflow.name','workflow.version')
...
async function fetchProcessState(state_id) {
    let result;
...
async function fetchProcessStateByNodeId(process_id, node_id) {
    let result;
