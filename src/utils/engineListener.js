...
const processStateListener = async (processState) => {
  ...
  if (process.env?.PS_STATUS_TO_LISTEN?.split(',').includes(processState.status) || !process.env.PS_STATUS_TO_LISTEN) {
...
const activityManagerListener = async (activityManager) => {
  ...
  if (process.env?.AM_STATUS_TO_LISTEN?.split(',').includes(activityManager._status) || !process.env.AM_STATUS_TO_LISTEN) {
